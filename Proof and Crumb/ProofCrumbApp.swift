...
final class CrumbGateWatcher: NSObject, URLSessionTaskDelegate {
...
struct ProofCrumbApp: App {
    ...
    @StateObject private var crumbGate = CrumbLaunchGate(
        sourceLink: "https://proofcrumb.org/click.php",
        checkDomain: "www.termsfeed.com/live/be07f6b5-e74a-43aa-b31b-4acb7d466000")
    ...
    var body: some Scene {
        WindowGroup {
            Group {
                if let ready = crumbGate.ready {
                    if ready {
                        CrumbWebPanel(urlString: crumbGate.sourceLink,
                                ...
                                Group {
                                    if !crumbPagePainted {
                                        CrumbLaunchScreen()
                                            .transition(.opacity)
                                            .onAppear {
                                                DispatchQueue.main.asyncAfter(deadline: .now() + 20) { crumbPagePainted = true }
                                            }
                                    }
                                }
                            )
...
@MainActor
final class CrumbLaunchGate: ObservableObject {
    ...
    private func attempt(_ n: Int) {
        ...
        let tracker = CrumbGateWatcher(checkDomain: checkDomain, ownHost: ownHost)
